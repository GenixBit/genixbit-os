...
# ─────────────────────────────────────────────────────────────────────────────
# Test 14: collect-migration-evidence.py must use d9aa0d2e Candidate 2 SHA
# ─────────────────────────────────────────────────────────────────────────────
info "Test 14: collect-migration-evidence.py must use correct d9aa0d2e Candidate 2 SHA..."
...
if grep -q '1cb79fbf66714ebc6a4f0789571664ab571a87749a75b9700d69acf8906e7669' "$COLLECT_PY" 2>/dev/null; then
    fail_test "collect-migration-evidence.py: contains wrong expected_cand_sha 1cb79fbf — must be d9aa0d2e"
fi
if ! grep -q 'd9aa0d2e850fdbcfb87beeaecb1ea2762a4d9522aa48d3bc6aa2bd0c6ee6f228' "$COLLECT_PY" 2>/dev/null; then
    fail_test "collect-migration-evidence.py: does not contain correct expected_cand_sha d9aa0d2e"
fi
pass_test "collect-migration-evidence.py: uses correct d9aa0d2e Candidate 2 SHA"
