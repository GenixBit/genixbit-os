...
get_key_val() {
    ...
    local target_key=$2
    local val
    val=$(grep -E "^${target_key}=" "$file" 2>/dev/null | head -n 1 | cut -d'=' -f2- || true)
    printf '%s' "$val"
}
...
EVIDENCE_CLASSIFICATION=$(get_key_val "$MANIFEST_FILE" "EVIDENCE_CLASSIFICATION")

# 1. RELEASE_VERSION is present
[[ -n "$RELEASE_VERSION" ]] || fail 'RELEASE_VERSION is missing or empty.'

# 2. Candidate branch is present.
...
# 4. Verify candidate git relationship.
#
# A live candidate-validation branch must still point exactly at the recorded SHA.
# An OFFICIAL_PRODUCTION_RELEASE is immutable historical evidence: its source SHA
# must remain reachable from the recorded branch, but the branch is expected to
# advance after release. Requiring main HEAD to stay pinned to a released SHA
# makes every later development commit invalidate a previously valid release.
branch_head=""
...
[[ "$branch_head" =~ ^[0-9a-f]{40}$ ]] || fail "Candidate branch $CANDIDATE_BRANCH does not exist locally or on origin."

if [[ "$EVIDENCE_CLASSIFICATION" == "OFFICIAL_PRODUCTION_RELEASE" ]]; then
    if ! git cat-file -e "${CANDIDATE_SHA}^{commit}" 2>/dev/null; then
        git fetch --quiet --no-tags origin "$CANDIDATE_SHA" 2>/dev/null || true
    fi
    if git cat-file -e "${CANDIDATE_SHA}^{commit}" 2>/dev/null; then
        git merge-base --is-ancestor "$CANDIDATE_SHA" "$branch_head" \
            || fail "Official release CANDIDATE_SHA ($CANDIDATE_SHA) is not reachable from $CANDIDATE_BRANCH HEAD ($branch_head)."
        pass "Official release source $CANDIDATE_SHA remains reachable from $CANDIDATE_BRANCH."
    elif [[ "$branch_head" != "$CANDIDATE_SHA" ]]; then
        fail "Cannot prove official release CANDIDATE_SHA ($CANDIDATE_SHA) is reachable from $CANDIDATE_BRANCH in this checkout."
    fi
elif [[ "$branch_head" != "$CANDIDATE_SHA" ]]; then
...
# 9. VALIDATION_STATUS check and retired-candidate enforcement.
